			     ...
			     uint8_t *data)
{
	...
	do { /* while(0) */
		...
		case CMD_DB_QUERY_ADDRESS:
			err = cmd_db_validate_slv_id(
				ADDR_SLV_ID(query_info->addr));
		...
		for (slv_id_idx = 0; slv_id_idx < CMD_DB_MAX_SLV_ID;
		     slv_id_idx++) {
			/* Skip over other slv_id's if valid hint was given */
			if ((valid_slv_id_hint == CMD_DB_SUCCESS) &&
			    (smem_query_db->slv_id_info[slv_id_idx].slv_id !=
			     query_info->slv_id)) {
				continue;
			}
			...
			for (entry_idx = 0;
			     entry_idx <
			     smem_query_db->slv_id_info[slv_id_idx].cnt;
			     entry_idx++) {
				/* Look for a matching entry based on query type */
				entry = (cmd_db_entry_header*)(smem_query_db->data
					+ smem_query_db->slv_id_info
						[slv_id_idx].header_offset
		...
		qtiseclib_cb_memcpy(query_result->res_id, query_info->res_id,
		...
		query_result->version =
			smem_query_db->slv_id_info[slv_id_idx].version;
	 ...
	 where extra data might not be handled by all client images */
			uint32_t cpy_len = (query_result->len > entry->len) ?
						   (entry->len) :
						   (query_result->len);
			qtiseclib_cb_memcpy(
				data,
				(smem_query_db->data +
				 smem_query_db->slv_id_info[slv_id_idx]
					 .data_offset +
				 entry->offset),
				cpy_len);
...
uint32_t cmd_db_query_addr(const char *res_id)
{
	...
	qtiseclib_cb_memcpy(query.res_id, res_id, sizeof(query.res_id));
 ...
 *          cmd_db_err_type.
 */
cmd_db_err_type cmd_db_query_aux_data(const char *res_id, uint8_t *len,
				      uint8_t *data)
{
	cmd_db_err_type err = CMD_DB_SUCCESS;
	cmd_db_query_info query = { { 0 } };
...
static void cmd_db_test(void)
{
	...
	for (size_t i = 0; i < 2; ++i) {
		printf("cmd_db_test_addr[%zu] = %u\n", i, cmd_db_test_addr[i]);
	}

	cmd_db_query_aux_data("cx.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for cx.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");

	cmd_db_query_aux_data("mx.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for mx.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");

	cmd_db_query_aux_data("ebi.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for ebi.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");

	cmd_db_query_aux_data("lcx.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for lcx.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");
	...
	cmd_db_query_aux_data("lmx.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for lmx.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");

	cmd_db_query_aux_data("gfx.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for gfx.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");
	...
	printf("cmd_db_test_data for mss.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");

	cmd_db_query_aux_data("ddr.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for ddr.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");

	cmd_db_query_aux_data("xo.lvl", &len, (uint8_t *)cmd_db_test_data);
	printf("cmd_db_test_data for xo.lvl = ");
	for (size_t i=0; i<16; ++i) {
		printf("%04X:", cmd_db_test_data[i]);
	}
	printf("\n");
	...
	for (size_t i = 0; i < 4; ++i) {
		printf("cmd_db_test_priority[%zu] = %u\n", i, cmd_db_test_priority[i]);
	}


	printf("ijhlk Finished testing\n");
}
...
cmd_db_err_type cmd_db_init(void)
{
	cmd_db_err_type err = CMD_DB_SUCCESS;
	uint32_t size = 0;
	smem_query_db = (cmd_db_header_type *)cmd_db_target_mem_init(&size);
