			       ...
			       u_register_t plat_params_from_bl2)
{
	...
	qti_console_ringbuf_register(&g_qti_console_ringbuf,
					(uintptr_t) &g_qti_bl31_ringbuf);
	g_qti_console_ringbuf.flags |= CONSOLE_FLAG_RUNTIME;

	//Write the location and size of the ring buffer to shared imem
	*(uint64_t*)TFA_BL31_SHARED_IMEM_RING_BUF_BASE =
		(uint64_t)(g_qti_bl31_ringbuf.rb_buffer);
	*(uint32_t*)TFA_BL31_SHARED_IMEM_RING_BUF_SIZE =
		(uint32_t)g_qti_bl31_ringbuf.rb_size;
