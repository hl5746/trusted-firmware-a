...
#define CORE_VERIFY(cond) assert(cond)
...
typedef struct pwr_utils_lvl_res_s {
	...
	uint16_t vlvls
		[SYS_RES_LVLS]; /* Supported vlvls with hlvl denoted by index */
...
void pwr_utils_lvl_init(void)
{
	...
	for (i = 0; i < g_res_count; i++) {
		...
		CORE_VERIFY(data_len > SYS_RES_LVLS);
		cmd_db_query_aux_data(res_name, &aux_len,
				      (uint8_t *)res->vlvls);
		CORE_VERIFY(data_len == (uint32_t)aux_len);

		/* Calculating actual levels this resource supports */
		for (j = 0; j < (data_len / sizeof(uint16_t)); j++) {
...
int pwr_utils_lvl_resource_idx(const char *res_name)
{
	...
	for (i = 0; i < g_res_count; i++) {
		if (qtiseclib_cb_strcmp(g_res[i].name, res_name) == 0) {
			return i;
		}
	}
...
int pwr_utils_vlvl(int resource_idx, int hlvl)
{
	...
	return ((size_t)hlvl < res->count ? res->vlvls[hlvl] :
					    RAIL_VOLTAGE_LEVEL_INVALID);
}
